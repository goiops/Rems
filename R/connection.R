...
connect <- function(usr, pwd, proxies = NULL, server = c('prod', 'cluster', 'stable', 'beta', 'nightly', 'ctc'), server_url = NULL)
  {
    ...
    if (is.null(proxies)) {
      ...
      r <- httr::POST(uri,
               httr::use_proxy(proxies$url,
                         port = proxies$port,
                         username = proxies$usr,
                         password = proxies$pwd),
               httr::add_headers(.headers = header),
               body = body,
               encode = "form")
    }
...
#' Arbitrary Request
#'
#' Perform an arbitrary GET, httr::POST or httr::DELETE function using a connection  object.
#'
#' For advanced users only, this function allows you to perform an arbitrary GET, httr::POST or httr::DELETE
...
#' @param rtype A string indicating the kind of HTTP request to be performed - accepts GET, httr::POST and httr::DELETE
...
#' @param headers A list of headers to be used by the GET/httr::POST/httr::DELETE request, per the \code{.headers} argument
...
#'
#' @return Returns the raw GET/httr::POST/httr::DELETE output from the request.
#'
#' @export
request <-
  function(conn, rtype = c("GET","httr::POST","httr::DELETE"), uri_keys = NULL, uri_args = NULL,
           ...
           )
  {
    ...
    rtype <- match.arg(rtype, c("GET","httr::POST","httr::DELETE"), several.ok = FALSE)

    if (rtype=="GET") {
      tryCatch({
        r <- GET(uri, prxy, query = body, httr::add_headers(.headers = headers), encode = encoding)
      }, error = function(err) {
        print(err)
